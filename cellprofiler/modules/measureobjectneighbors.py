...
class MeasureObjectNeighbors(cellprofiler.module.Module):
    ...
    def create_settings(self):
        self.object_name = cellprofiler.setting.ObjectNameSubscriber(
            ...
            doc='Select the objects whose neighbors you want to measure.')
...
""".format(**{
        ...
        self.distance = cellprofiler.setting.Integer(
            ...
            1,
            doc="""\
*(Used only when “{D_WITHIN}” is selected)*
...
"""
    ...
    def run(self, workspace):
        ...
        assert isinstance(neighbor_objects, cellprofiler.object.Objects)
        ...
        if self.neighbors_are_objects:
            ...
            neighbor_has_pixels = numpy.bincount(neighbor_objects.small_removed_segmented.ravel())[1:] > 0
        ...
        if self.distance_method == D_EXPAND:
            ...
            if dimensions == 2:
                ...
                k, i, j = scipy.ndimage.distance_transform_edt(labels == 0, return_distances=False, return_indices=True)
                labels = labels[k, i, j]
        ...
        if nneighbors > (1 if self.neighbors_are_objects else 0):
            ...
            ncenters = centrosome.cpmorphology.centers_of_labels(neighbor_objects.small_removed_segmented).transpose()
            areas = centrosome.cpmorphology.fixup_scipy_ndimage_result(
            ...
            if dimensions == 2:
                ...
                min_i = numpy.maximum(
                    centrosome.cpmorphology.fixup_scipy_ndimage_result(min_i) - distance,
                    0
                ).astype(int)

                max_i = numpy.minimum(
                    centrosome.cpmorphology.fixup_scipy_ndimage_result(max_i) + distance + 1,
                    labels.shape[0]
                ).astype(int)

                min_j = numpy.maximum(
                    centrosome.cpmorphology.fixup_scipy_ndimage_result(min_j) - distance,
                    0
                ).astype(int)

                max_j = numpy.minimum(
                    centrosome.cpmorphology.fixup_scipy_ndimage_result(max_j) + distance + 1,
                    labels.shape[1]
                ).astype(int)
            else:
                k, i, j = numpy.mgrid[0:labels.shape[0], 0:labels.shape[1], 0:labels.shape[2]]
                min_k, max_k, min_k_pos, max_k_pos = scipy.ndimage.extrema(k, labels, object_indexes)
                min_i, max_i, min_i_pos, max_i_pos = scipy.ndimage.extrema(i, labels, object_indexes)
                min_j, max_j, min_j_pos, max_j_pos = scipy.ndimage.extrema(j, labels, object_indexes)

                min_k = numpy.maximum(
                    centrosome.cpmorphology.fixup_scipy_ndimage_result(min_k) - distance,
                    0
                ).astype(int)

                max_k = numpy.minimum(
                    centrosome.cpmorphology.fixup_scipy_ndimage_result(max_k) + distance + 1,
                    labels.shape[0]
                ).astype(int)
            ...
            for object_number in object_numbers:
                ...
                if self.neighbors_are_objects:
                    ...
                    if dimensions == 2:
                        ...
                        outline_patch = perimeter_outlines[min_k[index]:max_k[index],
                                                           min_i[index]:max_i[index],
                                                           min_j[index]:max_j[index]] == object_number
    ...
    def get_measurement_objects(self, pipeline, object_name, category,
                                measurement):
        if (self.neighbors_are_objects or measurement not in self.get_measurements(pipeline, object_name, category)):
            return []
